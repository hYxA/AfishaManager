...
class AfishaManagerTestWithoutBeforeEach {
    ...
    AfishaManager manager10 = new AfishaManager(repository);
    ...
    @Test
    public void shouldGetOnly3Items() {
        manager.addFilm(first);
        manager.addFilm(second);
        manager.addFilm(third);
    ...
    @Test
    public void shouldGetOnly3ItemsFromManager10() {
        manager10.addFilm(first);
        manager10.addFilm(second);
        manager10.addFilm(third);
        FilmItem[] actual = manager10.getAll();
        FilmItem[] expected = {
                third,
                second,
                first
